...
# Remove unused fullText
content = content.replace('        let fullText = "";\n', '')
# Fix any
content = content.replace('function getErrorMessage(error: any): string {', 'function getErrorMessage(error: unknown): string {\n  const message = error instanceof Error ? error.message : String(error);')
...
# Fix Button 1 content (with -> 与)
content = content.replace('又解决普通垂直工具的质量 with 稳定性缺陷', '又解决普通垂直工具的质量与稳定性缺陷')

# Fix Button 3 content (with -> dengan)
content = content.replace('stabilitas buruk with server yang sering crash', 'stabilitas buruk dengan server yang sering crash')

# Fix other typos if any. I see 'dan' changed to 'and' in some places in my previous script?
# Let's check 'and' in button 3.
content = content.replace('akurat reproduce produk lebih dari 98%', 'tingkat akurasi reproduksi produk lebih dari 98%') # checking if I messed up more.
# Actually I'll just re-apply the full strings to be safe.

review_content = """第三方实测｜电商 AI 作图乱象丛生，电商卖家应该怎么选？
...
Saat ini, produksi visual e-commerce telah memasuki era AI. Namun bagi jutaan penjual, memilih alat yang tepat jauh lebih penting daripada sekadar menggunakan secara acak. Penulis, dalam kapasitas sebagai pihak ketiga, menghabiskan satu bulan untuk menguji 12 alat AI populer untuk pembuatan gambar, mencakup tipe umum dan vertikal khusus e-commerce. Hasilnya menunjukkan titik sakit industri meledak: AI umum tidak memahami e-commerce, alat vertikal sulit digunakan and tidak stabil, and hanya sedikit produk yang benar-benar memenuhi kebutuhan penjual.

AI umum seperti Midjourney and Stable Diffusion memang menghasilkan kualitas gambar memukau, tetapi sejatinya adalah alat seni digital and hampir tidak memahami aturan platform seperti Taobao atau Tokopedia. Masalah yang sering muncul: ukuran gambar tidak sesuai, produk tampak terdistorsi, warna menyimpang, and tingkat keberhasilan gambar kurang dari 30%. Lebih parah, konten yang dihasilkan tidak memiliki lisensi komersial, rawan sengketa hak cipta, and sama sekali tidak sesuai dengan kebutuhan kepatuhan e-commerce.
...
# Escape backticks and fix 'and' in solution
solution_content = solution_content.replace('and', 'dan')

content = re.sub(r'const useReviewExample = .*?const useSolutionExample = .*?setArtikelContoh\(.*?\);',
                 f'const useReviewExample = () => setArtikelContoh(`{review_content}`);\n  const useFeatureExample = () => setArtikelContoh(`{feature_content}`);\n  const useSolutionExample = () => setArtikelContoh(`{solution_content}`);',
