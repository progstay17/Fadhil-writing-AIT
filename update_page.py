...
# Replace useDefaultExample function definition
pattern = r"const useDefaultExample = \(\) => \{.*?  \};"
content = re.sub(pattern, functions, content, flags=re.DOTALL)

# Replace the button row
old_button = '<button onClick={useDefaultExample} className="text-[10px] text-blue-600 hover:underline">{t("buttons.use_sample")}</button>'
